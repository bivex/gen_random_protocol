...
class CSourceEmitter(CodeEmitter):
    ...
    def emit(self) -> str:
        ...
        return "\n".join([
            ...
            "    /*",
            "     * Strict-monotonic anti-replay check, safe across uint32 wraparound.",
            "     *",
            "     * Caller MUST initialise *last_seq = 0xFFFFFFFFu before the first frame",
            "     * of each session. The signed-difference test then:",
            "     *   - accepts forward frames (incl. across the 0xFFFFFFFF -> 0 rollover),",
            "     *   - rejects exact replays, stale, and out-of-order frames.",
            "     * This is STRICT-monotonic (no reorder window); for lossy or reordering",
            "     * transports use an IPsec-style sliding bitmap window instead.",
            "     *",
            "     * Note: this is an OPT-IN helper. nebula_slot_hdr_validate() does NOT call it",
            "     * (it is stateless); call this per-session after a successful hdr_validate.",
            "     */",
            "    if (!last_seq) return false;",
            "    int32_t delta = (int32_t)(incoming_seq - *last_seq);  /* wrap-safe signed delta */",
            "    if (delta > 0) {{",
