    ...
    def emit(self) -> str:
        ...
        return "\n".join([
            ...
            *(f"        case {m.name}: opcode_valid = true; break;" for m in p.messages),
            ...
            "    /*",
            "     * Sliding-window anti-replay (IPsec AH/ESP style, width",
            f"     * {pname}_REPLAY_WINDOW). Wraparound-safe via a signed difference.",
            "     *",
            "     *  - First frame             -> establish the window, accept.",
            "     *  - Forward frame           -> slide window right, accept.",
            "     *  - Within window, unseen   -> mark bit, accept (out-of-order OK).",
            "     *  - Within window, seen     -> reject (replay).",
            "     *  - Older than the window   -> reject (stale).",
            "     *",
            "     * Init: zero-initialise the state struct (e.g. {0}); one per session.",
            "     * Opt-in: hdr_validate() is stateless, so call this after every successful",
            "     * hdr_validate() to enforce replay protection.",
            "     */",
            "    if (!st) return false;",
            "    if (!st->initialized) {{",
            "        st->last_seq = seq;",
