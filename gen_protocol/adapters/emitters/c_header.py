...
class CHeaderEmitter(CodeEmitter):
    ...
    def _prototypes(self) -> str:
        ...
        return "\n".join([
            ...
            f"const void *payload, size_t payload_len);",
            f"",
            f"/* === Sliding-window anti-replay (IPsec-style, width {p.name}_REPLAY_WINDOW) === */",
            f"#define {p.name}_REPLAY_WINDOW 64",
            f"typedef struct {{",
            f"    uint32_t last_seq;    /* highest sequence seen */",
            f"    uint64_t window;      /* bit i set => sequence (last_seq - i) already received */",
            f"    bool     initialized; /* zero-init to false; first frame establishes the window */",
            f"}} {n}_replay_state_t;",
            f"bool {n}_replay_check({n}_replay_state_t *st, uint32_t seq);",
            f"",
            f"/* === Multi-Session Anti-Replay Table (Memory-Bounded against Session Exhaustion) === */",
            f"#ifndef {p.name}_REPLAY_TABLE_CAPACITY",
            f"#define {p.name}_REPLAY_TABLE_CAPACITY 1024U",
            f"#endif",
            f"typedef struct {{",
            f"    uint32_t           session_id;",
            f"    {n}_replay_state_t state;",
            f"    uint32_t           last_access;  /* Monotonic clock for LRU eviction */",
            f"    bool               occupied;",
            f"}} {n}_replay_entry_t;",
            f"typedef struct {{",
            f"    {n}_replay_entry_t entries[{p.name}_REPLAY_TABLE_CAPACITY];",
            f"    size_t             count;",
            f"    uint32_t           access_clock;",
            f"}} {n}_replay_table_t;",
